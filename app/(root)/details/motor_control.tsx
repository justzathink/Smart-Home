...
import { fetchMOTORStatus, updateMOTORStatus } from "@/api/motor_control";
...
const LightControl: React.FC = () => {
    const [devices, setDevices] = useState<Device[]>([
        { id: "1", label: "Máy bơm 1", icon: icons.lightbulb },
    ]);
    ...
    const toggleSwitch = async (deviceId: string) => {
        ...
        console.log(!status);
        await updateMOTORStatus(!status);
    };
    ...
    return (
        <SafeAreaView className='px-6 bg-white h-full'>
            ...
            <Text className='font-semibold text-2xl mb-3'>Danh sách máy bơm</Text>
            <FlatList
                data={devices}
