...
const DeviceControl: React.FC<DeviceControlProps> = ({ icon, label, isEnabled, toggleSwitch }) => {
    return (
...
const LightControl: React.FC = () => {
    ...
    const toggleSwitch = async (deviceId: string, newValue: boolean) => {
        setDeviceStates(prev => ({ ...prev, [deviceId]: newValue }));
        await updateLEDStatus(newValue);
    };
